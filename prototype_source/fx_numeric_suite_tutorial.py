...
and its corresponding quantized model to know where are they differ most.
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# The first analysis we can do is comparing the weights of fp32 model and int8
# model by calculating the SQNR between each pair of weights.
#
# The extract_weights API knows how to extract weights from linear, convolution
# and LSTM layers. It works for dynamic quantization as well as PTQ/QAT.
...
#
# The add_loggers/extract_logger_info API knows how to extract activations
# from any layer with a torch.Tensor return type. It works for dynamic
# quantization as well as PTQ/QAT.
